...
import { getFirestore, collection, doc, getDoc, getDocs, setDoc } from 'https://www.gstatic.com/firebasejs/10.14.1/firebase-firestore.js';
...
window.firebasePortal={
  ...
  async createRequest(entry){
    ...
    await setDoc(doc(db,'solicitacoes',entry.codigo),{...entry,ownerUid:user.uid});
    return entry.codigo;
  },
  async lookupOwnRequest(code){
    await ensureIdentity();
    const snap=await getDoc(doc(db,'solicitacoes',code));
  ...
  async updateRequest(code,item){
    await ensureIdentity();
    // setDoc replaces removed service-specific fields; immutable ownerUid is retained by edit form.
    await setDoc(doc(db,'solicitacoes',code),{...item});
  },
  async loadConfig(){
    await ensureIdentity();
